...
| `admin_list_accounts.rs` | `GET /v1/admin/accounts` — operator account listing/search: DID-ordered cursor pagination (`limit` default 50 max 100, `cursor` = last DID), derived-lifecycle `status` filter (`active`/`deactivated`/`suspended`/`takendown`; unknown value → 400), literal handle/DID substring `q` search, every row carrying `totalBytes` + `quotaUsedPct` (against the response-level `quotaBytes`) so the companion app renders a per-account capacity readout in one request per page. Includes accounts in every lifecycle state and without a repo. Admin-authed via `require_admin`; the signature covers the bare path, so paging/filter params vary without re-signing |
| `admin_health.rs` | `GET /v1/admin/health` — operator server-health readout for the companion app's Status screen: version/uptime, account counts by derived lifecycle, physical blob/block counts + bytes, firehose state (current seq, subscribers, retained events, backfill-window age — `null` on an empty log), and each background sweep's last completed pass from `sweep_status.rs` (`null` until its first pass after boot). Literal facts only — no ok/warn verdicts (thresholds are the client's judgment, not API shape). The JSON counterpart of `GET /metrics` (OTel instruments are write-only in-process). Admin-authed via `require_admin` |
