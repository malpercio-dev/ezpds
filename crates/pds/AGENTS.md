...
| `spaces.rs` | `spaces` rows (V065): every space this PDS interacts with, keyed by canonical URI, plus the `space_members` membership check behind credential issuance; the rest of the member/notify queries land with their consuming surfaces — see module doc |
