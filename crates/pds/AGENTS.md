...
| `password_reset.rs` | `insert_reset_token`, `get_reset_token`, `mark_reset_token_used`, `update_password_hash`; the admin repair route inserts the same one-hour hashed token envelope inside its audit transaction |
