...
| `resolve_identity.rs` | `GET /xrpc/com.atproto.identity.resolveDid`, `GET /xrpc/com.atproto.identity.resolveIdentity`, `POST /xrpc/com.atproto.identity.refreshIdentity` — shared `identity_resolution` local→network resolution chain (DID document + bidirectionally-verified handle) behind all three; spec-shaped responses. `resolveDid`/`resolveIdentity` are cache-first; `refreshIdentity` force-refreshes (re-resolves from the authoritative source and rewrites the cache row — its "purge caches, re-resolve, return the fresh view" contract) and then, when this PDS hosts the account (`account_exists`), emits an `#identity` firehose frame so relays re-resolve — method-agnostic, and the leg that propagates an externally-hosted `did:web` `did.json` edit onto the network |
