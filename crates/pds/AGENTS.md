...
| `extractors.rs` | Imperative Shell | `AuthenticatedUser` axum extractor + the shared `authenticate_access(headers, method, uri, state)` helper it wraps — the single authoritative access-auth path (extract Bearer/DPoP token → verify → RFC 9449 scheme ↔ `cnf.jkt` binding rules → DPoP-proof validation). The repo-write handlers (createRecord/putRecord/deleteRecord/applyWrites) call it directly (they resolve + open the target repo around the auth check, so they can't use the extractor), so their binding enforcement can't drift — a DPoP-bound token presented as plain Bearer with no proof is rejected on repo writes exactly as on the extractor |
| `guards.rs` | Imperative Shell | Route-level auth guards: `require_admin`/`require_admin_token`/`require_admin_json` (master token OR signed companion-device request), `require_session`, `require_pending_session`, `require_device_token`, and `authenticate_account_owner` (the account-holder agent surfaces' dual credential — wallet session token first, then a full-access OAuth/XRPC access token; agent-derived and app-password tokens refused; returns a vocab-neutral `OwnerAuthError` that `/v1/agents/*` maps to XRPC `ApiError`s and the claim-confirm gate maps to auth.md errors); the admin-device request-signing envelope + self-signature verification. Reads request headers and queries `sessions`/`devices`/`admin_devices`; owns no schema |
