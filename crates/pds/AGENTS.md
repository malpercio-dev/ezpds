...
| `spaces.rs` | `spaces` rows (V065): every space this PDS interacts with, keyed by canonical URI — `insert_space` (record a foreign space on a member's write, never touching an existing row) vs `create_space` (simplespace create: claims/revives a config-less row), config set/clear, tombstoning — plus the `space_members` list (membership check behind credential issuance, add/remove/list/clear) and the space-scoped notify-registration clear; the remaining notify queries land with the space-host surface — see module doc |
...
| `space_views.rs` | shared handler-free support for the space routes (routes may not import one another): space-ref parsing, the `validate`-flag record check, stored-block decoding, the write-result shape, and the simplespace config's lexicon-union ↔ column mapping (`policy_from_lex`/`app_access_from_lex` refuse the open-union members this host does not implement) |
| `simplespace_create_space.rs` | `POST /xrpc/com.atproto.simplespace.createSpace` — a space anchored on the caller's own DID (`at://{did}/space/{type}/{skey}`, TID skey when absent); `manage=create`; refuses `managing-app`/`allowList` (`UnsupportedPolicy`/`UnsupportedAppAccess`); a deleted URI may be created again |
| `simplespace_update_space.rs` | `POST /xrpc/com.atproto.simplespace.updateSpace` — each supplied union replaces its axis wholesale; `manage=update`, owner only |
| `simplespace_delete_space.rs` | `POST /xrpc/com.atproto.simplespace.deleteSpace` — tombstones the row (config cleared, `deleted_at` set so `getSpaceCredential` answers `SpaceDeleted`), drops the member list + notify registrations + the authority's own repo; other members' repos stay, unreadable; idempotent; `manage=delete`, owner only |
| `simplespace_get_space.rs` | `GET /xrpc/com.atproto.simplespace.getSpace` — `{uri, policy, appAccess}` as lexicon unions; the one simplespace method a DPoP-bound space credential reaches (`auth::space::authenticate_space_access`); an OAuth caller needs `read_self` and must be the authority |
| `simplespace_add_member.rs` / `simplespace_remove_member.rs` | `POST /xrpc/com.atproto.simplespace.{addMember,removeMember}` — the host-internal member list behind the `member-list` policy; `manage=update`, owner only, idempotent |
| `simplespace_list_members.rs` | `GET /xrpc/com.atproto.simplespace.listMembers` — DID-keyset paged; OAuth `read_self` only (never a space credential), owner only |
