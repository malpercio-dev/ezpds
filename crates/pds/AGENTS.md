...
| `oauth_errors.rs` | Shared `OAuthTokenError` — the RFC 6749 §5.2 `{error, error_description}` responder (with the `use_dpop_nonce` header path) used by `oauth_token.rs` and `oauth_revoke.rs` (Functional Core, no handler) |
