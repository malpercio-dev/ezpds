...
| `kek.rs` | the exhaustive inventory of KEK-wrapped columns (`SecretFamily::ALL`: `signing_keys`, `reserved_signing_keys`, `pending_accounts.repo_signing_private_key_encrypted`, `relay_signing_keys`, `oauth_signing_key`, `jwt_signing_secret`, `iroh_identity`) with generic fetch/update over the ciphertext, plus the `kek_generation` marker in `server_metadata`. Consumed by `rewrap.rs`; a new KEK-wrapped column must be added as a variant here so master-key rotation covers it |
