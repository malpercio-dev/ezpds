...
| `space.rs` | Mixed (unavoidable) | Atproto Spaces token auth (proposal 0016): `SpaceRef` (the `space-ref` format), delegation-token mint/verify (single-use via `space_jti_replay`), DPoP-bound space-credential mint/verify (`#atproto_space` → `#atproto` key fallback), the issuance policy, and `authenticate_space_read` — the ONE read-side seam (covering OAuth grant *or* DPoP-bound credential, never bearer, full RFC 9449 proof validation + per-host `jti` replay); `just space-auth-seam-check` freezes it — see module doc |
