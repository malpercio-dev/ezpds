...
| `resolve_identity.rs` | `GET /xrpc/com.atproto.identity.resolveDid`, `GET /xrpc/com.atproto.identity.resolveIdentity`, `POST /xrpc/com.atproto.identity.refreshIdentity` — shared `identity_resolution` local→network resolution chain (DID document + bidirectionally-verified handle) behind all three; spec-shaped responses. `resolveDid`/`resolveIdentity` are cache-first; `refreshIdentity` force-refreshes (re-resolves from the authoritative source and rewrites the cache row — its "purge caches, re-resolve, return the fresh view" contract) |
...
| `request_plc_operation_signature.rs` | `POST /xrpc/com.atproto.identity.requestPlcOperationSignature` — mint a single-use 1-hour email token authorizing a later `signPlcOperation` (interop migration path). Full-access-authed; the token is emailed to the account address via `email::EmailSender` |
