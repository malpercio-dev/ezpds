...
| `proxy.rs` | (2026-07-14 split from `resolution.rs`, ~450 LOC) the `atproto-proxy` header target guard: `resolve_atproto_proxy_target` resolves a caller-supplied `<did>#<serviceId>` header to an upstream service endpoint — honored for any proxied namespace naming an explicit header target, mandatory for `com.atproto.moderation.*` (no configured default) — then `validate_proxy_endpoint` SSRF-checks the advertised `serviceEndpoint` (scheme, userinfo, query/fragment, public-address-only on both IP literals and resolved domains) before it's ever handed to the HTTP client. `build_pinned_client`/`PinnedResolution` close the redirect/re-resolution TOCTOU gap between that check and connect time. Security-critical — the only thing standing between an authenticated request and an SSRF into the PDS's private network — kept in its own reviewable file; also reused by `auth::permission_sets`' Lexicon-authority fetch |
