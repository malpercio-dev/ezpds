...
- **V028**: Adds the `repo_seq` table — the **persistent firehose event log** backing
  ...
  `#commit` or `#account` frame) is now persisted here *before* being broadcast, so `seq` is
...
- `migrations/V028__repo_seq.sql` - Adds `repo_seq`, the persistent firehose event log behind `com.atproto.sync.subscribeRepos`. `seq INTEGER PRIMARY KEY` (explicit, sequencer-assigned, not AUTOINCREMENT), `did`, `event_type` (`commit`|`account`), `event` BLOB (DAG-CBOR payload reconstructing the wire frame, incl. the commit CAR `blocks`), `sequenced_at`. Persisted before broadcast so `seq` survives restarts and cursor replay reads from the log; the `firehose_gc` retention sweep prunes old rows below the live frontier, keeping `MAX(seq)` and a dense retained suffix so a cursor inside the pruned window degrades to best-effort replay
