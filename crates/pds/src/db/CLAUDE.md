...
- `transfers.rs` - Query functions for V027: `insert_transfer` opens a `pending` transfer (sweep-then-insert), returning `InitiateOutcome::{Created, DuplicateActive, CodeCollision}` — a `did` index violation is the per-account 409 path, a `code` violation triggers the caller's regenerate-and-retry
