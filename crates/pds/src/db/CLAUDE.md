...
- `firehose_seq.rs` - Query functions for V028: `max_seq` (startup seed for the sequence counter), `insert_event` (append with an explicit `seq`), `events_in_range(after, upper, limit)` (the cursor-replay page query — `after < seq <= upper`, oldest first), `retained_boundary_seq` (the age-based retention watermark: first *retained* seq, over-prune-safe), `exists_at_or_below` (is the cursor inside the retained range or below a pruned prefix? used by replay), `prune_below` (the sweep delete). Consumed by `firehose.rs` (the sequencer + replay), `routes/sync_subscribe_repos.rs` (replay paging), and `firehose_gc.rs` (retention)
