...
## Latest Updates
- **V025**: Adds the admin-device data model for the operator companion app — three tables: `admin_pairing_codes` (single-use, short-TTL enrollment codes), `admin_devices` (per-device public keys as `did:key`, with a `scopes` growth hook defaulting to `full`), and `admin_nonces` (seen request nonces for anti-replay, keyed `(device_id, nonce)` so uniqueness is per-device, FK→`admin_devices`). Status is derived, not stored (matching `claim_codes` V004): a pairing code is *pending* while `consumed_at IS NULL AND expires_at > now`; a device is *active* while `revoked_at IS NULL`. Query functions live in `admin_devices.rs`; the pairing/register routes (`routes/admin_devices.rs`, Phase 3) wire the mint/get/consume + insert functions. The Phase 4 signed-request guard (`require_admin` in `routes/auth.rs`) consumes `get_device`/`insert_nonce_if_absent`/`touch_last_seen` (the last bumps `last_seen_at` on a successful device auth). The Phase 5 list/revoke device-management routes (`routes/admin_devices.rs`: `GET /v1/admin/devices`, `POST /v1/admin/devices/:id/revoke`) consume `list_devices`/`revoke_device`/`get_device`.
