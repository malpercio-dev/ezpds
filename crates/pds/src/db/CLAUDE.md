...
- **V028**: Adds the `repo_seq` table — the **persistent firehose event log** backing
  ...
  never the live frontier; `age_cutoff_seq`/`prune_below` do the SQL). Query functions live in
  `firehose_seq.rs`.
...
- `firehose_seq.rs` - Query functions for V028: `max_seq` (startup seed for the sequence counter), `insert_event` (append with an explicit `seq`), `events_in_range(after, upper, limit)` (the cursor-replay page query — `after < seq <= upper`, oldest first), `age_cutoff_seq` (the age-based retention watermark), `prune_below` (the sweep delete). Consumed by `firehose.rs` (the sequencer), `routes/sync_subscribe_repos.rs` (replay paging), and `firehose_gc.rs` (retention)
