...
- **V028**: Adds the `repo_seq` table — the **persistent firehose event log** backing
  ...
  `value` is not stored (not on the wire). Append-only and currently unbounded — pruning is future
  work. Query functions live in `firehose_seq.rs`.
...
- `migrations/V028__repo_seq.sql` - Adds `repo_seq`, the persistent firehose event log behind `com.atproto.sync.subscribeRepos`. `seq INTEGER PRIMARY KEY` (explicit, sequencer-assigned, not AUTOINCREMENT), `did`, `event_type` (`commit`|`account`), `event` BLOB (DAG-CBOR payload reconstructing the wire frame, incl. the commit CAR `blocks`), `sequenced_at`. Persisted before broadcast so `seq` survives restarts and cursor replay reads from the log; append-only, unbounded (pruning is future work)
- `firehose_seq.rs` - Query functions for V028: `max_seq` (startup seed for the sequence counter), `insert_event` (append with an explicit `seq`), `events_in_range(after, upper, limit)` (the cursor-replay page query — `after < seq <= upper`, oldest first). Consumed by `firehose.rs` (the sequencer) and `routes/sync_subscribe_repos.rs` (replay paging)
