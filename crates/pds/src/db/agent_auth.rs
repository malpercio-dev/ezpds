...
/// The `agent_identities` SELECT projection. A struct rather than a tuple because sqlx only
/// implements `FromRow` for tuples up to 16 elements and this table has outgrown that; field
/// names must match the selected column names, which `FromRow` binds by name.
#[derive(sqlx::FromRow)]
struct IdentitySqlRow {
    id: String,
    did: Option<String>,
    ...
    email: Option<String>,
    scopes: String,
    identity_assertion: Option<String>,
    assertion_expires_at: String,
    pre_claim_scopes: Option<String>,
    claim_token: Option<String>,
    claim_token_expires_at: Option<String>,
    status: String,
    created_at: String,
    updated_at: String,
...
pub(crate) struct AgentIdentityRow {
    ...
    /// The handle an `anonymous` agent proposed for a child account at registration (V071).
    /// Advisory only — the claim-approval screen shows it, the user decides.
    pub(crate) handle_hint: Option<String>,
}
...
pub(crate) struct NewAgentIdentity<'a> {
    ...
    pub(crate) handle_hint: Option<&'a str>,
}
    ...
    let result = sqlx::query(
        "INSERT INTO agent_identities \
         (id, did, parent_did, registration_type, issuer, subject, email, scopes, identity_assertion, \
          assertion_expires_at, pre_claim_scopes, claim_token, claim_token_expires_at, handle_hint, \
...
pub(crate) async fn list_children_of_parent(
    ...
    let rows = sqlx::query_as::<_, IdentitySqlRow>(
        "SELECT id, did, parent_did, registration_type, issuer, subject, email, scopes, \
                ...
                claim_token_expires_at, status, created_at, updated_at, handle_hint \
...
pub(crate) async fn get_agent_identity_by_issuer_subject(
    ...
    let row = sqlx::query_as::<_, IdentitySqlRow>(
        "SELECT id, did, parent_did, registration_type, issuer, subject, email, scopes, identity_assertion, \
                ...
                status, created_at, updated_at, handle_hint \
...
/// Convert an ownerless `anonymous` registration into a `child` one, in place.
///
/// The cooperative mint arm of the claim ceremony: rather than opening a second registration for
/// the freshly minted child, the row the agent already polls becomes the child's — so its
/// `claim_token`, registration id, and audit trail all carry through, and the claim-grant poll
/// returns a child-subject credential with no agent-side change.
///
/// The guard (`status = 'active' AND did IS NULL AND registration_type = 'anonymous'`) makes a lost
/// race roll back rather than re-home an already-claimed registration. All three of `did`,
/// `parent_did`, and `registration_type` move together because V047's CHECK constraint requires a
/// `child` row to carry both DIDs.
#[allow(clippy::too_many_arguments)]
pub(crate) async fn convert_identity_to_child<'e, E>(
    executor: E,
    id: &str,
    child_did: &str,
    parent_did: &str,
    ...
    let result = sqlx::query(
        "UPDATE agent_identities \
         SET status = 'claimed', \
             ...
             assertion_expires_at = ?, \
             updated_at = datetime('now') \
         WHERE id = ? AND status = 'active' AND did IS NULL AND registration_type = 'anonymous'",
    )
    .bind(child_did)
    .bind(parent_did)
    .bind(child_did)
...
fn into_identity_row(row: IdentitySqlRow) -> AgentIdentityRow {
    AgentIdentityRow {
        ...
        registration_type: RegistrationType::from_string(row.registration_type),
        issuer: row.issuer,
        subject: row.subject,
        email: row.email,
        scopes: row.scopes,
        identity_assertion: row.identity_assertion,
        assertion_expires_at: row.assertion_expires_at,
        pre_claim_scopes: row.pre_claim_scopes,
        claim_token: row.claim_token,
        claim_token_expires_at: row.claim_token_expires_at,
        status: AgentIdentityStatus::from_string(row.status),
