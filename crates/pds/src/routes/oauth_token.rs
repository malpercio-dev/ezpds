...
pub async fn post_token(
    ...
    match grant_type {
        ...
        "urn:workos:agent-auth:grant-type:claim" => handle_claim_polling(&state, form).await,
...
/// Successful claim-polling response body. Like [`JwtBearerTokenResponse`] it carries a plain Bearer
...
async fn handle_claim_polling(state: &AppState, form: TokenRequestForm) -> Response {
    cleanup_expired_state(state).await;
...
mod tests {
    ...
    // ── claim-polling grant (auth.md Step 4c) ─────────────────────────────────
