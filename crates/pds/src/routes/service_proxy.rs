...
//! `read_after_write::pipethrough_munged` (buffered munge: merges the requester's unindexed
//! records, applies the fallback ladder, emits `Atproto-Upstream-Lag`) only when **no** header
//! is present. The munge path hardcodes the configured AppView internally, so an explicit
//! header bypasses it and streams to the header target via `proxy_xrpc`; all other unmunged
//! `app.bsky.*` NSIDs always stream through `proxy_xrpc` (no buffering). `chat.bsky.*` requires
//! full access or a *privileged* app password regardless of target (plain `com.atproto.appPass`
//! is a 403).
//!
