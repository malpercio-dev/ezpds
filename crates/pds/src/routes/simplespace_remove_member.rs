...
//! com.atproto.simplespace.removeMember — remove a DID from a space's member list.
...
/// member is not notified (their PDS materializes a repo on first write). Idempotent: removing a non-member is a 200.
pub async fn simplespace_remove_member(
    State(state): State<AppState>,
    method: Method,
    uri: Uri,
    headers: HeaderMap,
    LexiconInput(input): LexiconInput<RemoveMemberInput>,
