...
// app-password token — privileged or not — never reads or writes these, with one deliberate
// divergence: an app password minted with the personal-details grant (ADR-0033,
// `AuthenticatedUser::can_manage_personal_details`) is admitted like a full session.
// Declaring a preference type here only ever narrows what an ungranted app password can see
// or manage, never what a full access-scope token can.
