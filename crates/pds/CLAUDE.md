...
| `claim_codes.rs` | invite/claim-code store (`claim_codes`, V004; rebuilt V041 with `revoked_at` + stable `id` cursor): `claim_code_valid` (preflight exists/unredeemed/unrevoked/unexpired check — the authoritative single-use redemption is a separate atomic UPDATE inside the account-creation transaction) + `mint_claim_codes` (batch-generate and persist unique codes, retrying on rare collisions, `MintClaimCodesError::Exhausted` after too many) + `list_claim_codes` (newest-first id-cursor inventory page) + `revoke_claim_code` (tombstone `revoked_at` iff unredeemed+unrevoked; outcome enum Revoked/AlreadyRevoked/Redeemed/NotFound) |
...
| `claim_codes.rs` | `POST /v1/accounts/claim-codes` (admin mint), `GET /v1/accounts/claim-codes` (admin inventory: full mint history newest-first, stable `id` cursor, derived status pending/redeemed/expired/revoked — terminal events win over the clock), `POST /v1/accounts/claim-codes/revoke` (admin revoke; code in the signed JSON body, not the path; idempotent for already-revoked, 409 for redeemed, 404 unknown) |
