...
| `claim_codes.rs` | `POST /v1/accounts/claim-codes` (admin mint), `GET /v1/accounts/claim-codes` (admin inventory: full mint history newest-first, rowid cursor, derived status pending/redeemed/expired/revoked — terminal events win over the clock), `POST /v1/accounts/claim-codes/revoke` (admin revoke; code in the signed JSON body, not the path; idempotent for already-revoked, 409 for redeemed, 404 unknown) |
