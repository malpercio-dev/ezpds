...
src/
  ...
  admin_nonce_sweep.rs— periodic sweep deleting stale `admin_nonces` rows via `db::admin_devices::sweep_stale_nonces` (template: account_reaper.rs; pure storage reclamation — the `(device_id, nonce)` primary key enforces replay rejection regardless; interval: `[admin_devices] nonce_sweep_interval_secs` / `EZPDS_ADMIN_DEVICES_NONCE_SWEEP_INTERVAL_SECS`, retention: `nonce_max_age_secs` / `EZPDS_ADMIN_DEVICES_NONCE_MAX_AGE_SECS`)
