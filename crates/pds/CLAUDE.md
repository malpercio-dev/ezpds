...
| `agent_event.rs` | `POST /agent/event/notify` — the auth.md `events_endpoint` (SET receiver). Accepts a **Security Event Token** (RFC 8417) pushed by a trusted identity provider (`application/secevent+jwt`, RFC 8935) and revokes the matching `identity_assertion` registration — the provider-initiated counterpart to `agents.rs`'s owner-driven revoke. Verified against the **same** `[agent_auth] trusted_issuers` list as ID-JAGs (`auth/issuer_trust.rs`), so gating is implicit: no trusted issuers → every SET is `invalid_issuer`. The SET's `iss`+`sub` (top-level, or a `subject` in the event payload) key the `(issuer, subject)` lookup; its `events` claim must carry `REVOKED_EVENT_TYPE`. Revocation reuses `revoke_agent_identity` + a `Revoked` audit event (`detail.source = "provider_set"`) atomically, exactly like `agents.rs::revoke_agent`. Idempotent: an unknown/already-revoked subject is still `202` (no oracle, replay-safe, no `jti` store). Errors use the RFC 8935 `{err, description}` body (`invalid_request`/`invalid_issuer`/`authentication_failed`), distinct from the XRPC and auth.md envelopes |
