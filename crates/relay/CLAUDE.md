...
src/
  ...
  iroh_tunnel.rs   — Iroh QUIC endpoint: NAT-traversing device↔relay tunnel (opt-in)
...
holds the bound endpoint and its node-id string; `get_device_relay` advertises that node id.
