...
//! only). A Bearer client never touches a DPoP keypair — there is nothing to construct or
//! load for it.
//!
...
//! into `NotAuthenticated`, so the caller's status classifier can surface the server's real
//! status and body. Non-400 statuses pass through untouched.
//!
//! **Breadcrumbs and token persistence are injected**, not hardcoded: this crate has no
//! Tauri/Keychain dependency, so a caller supplies a [`TransportObserver`] (defaulted to a
//! no-op for `new_bearer`'s common 3-arg form — see [`OAuthClient::new_bearer_with_observer`]
//! for callers that want real breadcrumbs) and, for DPoP-mode token refresh, a
//! [`TokenPersister`].
//!
//! Request methods: `get`, `post`/`put` (JSON), `post_no_body` (zero bytes and no
//! `Content-Type` — for no-input XRPC procedures like `requestPlcOperationSignature`/
//! `activateAccount`, whose lexicons define no input and where a spec-strict PDS rejects any
//! body), and `post_bytes` (raw byte body — CAR repo import and blob upload).
...
use crate::dpop::{DpopError, DpopKeypair};
use crate::error::{NoopObserver, TransportObserver};

/// Error type for all OAuth-related operations.
///
/// Variants serialize as `{ "code": "SCREAMING_SNAKE_CASE" }` to match the existing
/// error pattern (`CreateAccountError`, `DeviceKeyError`, etc.).
...
impl From<DpopError> for OAuthError {
    fn from(e: DpopError) -> Self {
        match e {
            DpopError::KeyGenFailed => OAuthError::DpopKeyGenFailed,
            DpopError::KeyInvalid => OAuthError::DpopKeyInvalid,
            DpopError::ProofFailed => OAuthError::DpopProofFailed,
            DpopError::KeychainError { .. } => OAuthError::KeychainError,
        }
    }
}

/// Persists a refreshed DPoP-mode token pair. Bearer-mode refresh never calls this — it
/// updates its session in memory only, by design (see [`OAuthClient::refresh_token`]'s doc).
pub trait TokenPersister: Send + Sync {
    fn store_tokens(&self, access_token: &str, refresh_token: &str) -> Result<(), String>;
}

/// A [`TokenPersister`] that persists nothing (used by [`OAuthClient::new`] callers that
/// manage persistence themselves, and by tests).
pub struct NoopTokenPersister;

impl TokenPersister for NoopTokenPersister {
    fn store_tokens(&self, _access_token: &str, _refresh_token: &str) -> Result<(), String> {
        Ok(())
    }
}

/// Active OAuth session carried by an [`OAuthClient`].
#[derive(Clone)]
...
/// A DPoP-mode token response, e.g. from `POST /oauth/token`.
#[derive(Debug, Deserialize)]
...
/// body — so the caller's status classifier can turn it into a `RateLimited` / `Unauthorized` /
/// `XrpcError` instead of this transport layer flattening it into `NotAuthenticated`.
///
...
/// refresh via `/oauth/token`). `Bearer` is the legacy session mode used for a plain
/// `accessJwt`/`refreshJwt` session (e.g. a migration destination account, or a
/// sovereign/password login) — no DPoP keypair is constructed for it.
...
pub struct OAuthClient {
    inner: Client,
    /// `Some` only in `AuthMode::Dpop`; Bearer mode never constructs or reads a keypair.
    dpop: Option<DpopKeypair>,
    /// The OAuth client_id sent on DPoP-mode token refresh. Unused in Bearer mode.
    client_id: String,
    session: Arc<Mutex<OAuthSession>>,
    base_url: String,
    auth_mode: AuthMode,
    observer: Arc<dyn TransportObserver>,
    persister: Arc<dyn TokenPersister>,
}

impl OAuthClient {
    /// Construct a DPoP-mode client from an existing session and a caller-loaded keypair.
    ///
    /// `Client::new()` inherits the TLS backend configured at the crate level via Cargo features
    /// (`default-features = false, features = ["rustls-tls"]`). No builder configuration is
    /// needed — the feature flags apply crate-wide, not per-client-instance.
    pub fn new(
        dpop: DpopKeypair,
        client_id: String,
        session: Arc<Mutex<OAuthSession>>,
    ...
    /// Build a Bearer-session client for a plain `accessJwt`/`refreshJwt` session (a migration
    /// destination account, a sovereign/password login, …). `expires_at` is derived from the
    /// access token's `exp` claim so proactive refresh works. Records no diagnostics breadcrumbs
    /// on transport failure — use [`Self::new_bearer_with_observer`] for that.
    pub fn new_bearer(
        access_jwt: String,
        refresh_jwt: String,
        base_url: String,
    ) -> Result<Self, OAuthError> {
        Self::new_bearer_with_observer(access_jwt, refresh_jwt, base_url, Arc::new(NoopObserver))
    }

    /// [`Self::new_bearer`], recording transport-failure breadcrumbs into `observer`.
    pub fn new_bearer_with_observer(
        access_jwt: String,
        refresh_jwt: String,
        base_url: String,
        observer: Arc<dyn TransportObserver>,
        ...
        Ok(Self {
            ...
            observer,
            persister: Arc::new(NoopTokenPersister),
        })
    }
    ...
    /// The DPoP keypair. Panics if called in Bearer mode — an internal invariant violation,
    /// since only DPoP-mode code paths call this.
    fn dpop(&self) -> &DpopKeypair {
        self.dpop
            .as_ref()
            .expect("dpop() called on a Bearer-mode OAuthClient")
    }
    ...
    async fn send_with_dpop<B: Serialize + Sync>(
        ...
        builder.send().await.map_err(|e| {
            ...
            self.observer
                .record_transport("oauthRequest", Some(url), &e);
    ...
    async fn send_bytes(
        ...
        builder.body(body.to_vec()).send().await.map_err(|e| {
            ...
            self.observer.record_transport("oauthUpload", Some(url), &e);
            OAuthError::NotAuthenticated
        })
    }
    ...
    /// session + persists via `persister`.
    ///
    /// For Bearer mode: POSTs `/xrpc/com.atproto.server.refreshSession` with the Bearer token,
    /// updates session **in memory only** (never calls `persister`, by design). Surfaces all
    ...
    async fn refresh_token_dpop(&self) -> Result<(), OAuthError> {
        ...
        let resp = self
            ...
            .map_err(|e| {
                self.observer
                    .record_transport("oauthRefresh", Some(&token_htu), &e);
                let e = e.without_url();
        ...
        if resp.status().as_u16() == 400 {
            ...
            if let Some(nonce_val) = retry_nonce {
                ...
                let resp2 = self
                    ...
                    .await
                    .map_err(|e| {
                        self.observer.record_transport(
    ...
    ///
    /// Updates session **in memory only** — never calls `persister`.
    /// Bearer refresh is only used for non-DPoP sessions and must not corrupt any
    /// separately-persisted DPoP session state.
    async fn refresh_token_bearer(&self) -> Result<(), OAuthError> {
        ...
        let resp = self
            ...
            .map_err(|e| {
                self.observer
                    .record_transport("oauthBearerRefresh", Some(&url), &e);
                let e = e.without_url();
        ...
        // Bearer refresh must not persist. Update session in memory only.
    ...
    /// Deserialize a 200 token response and update session + persist via `persister`.
    async fn apply_token_response(&self, resp: Response) -> Result<(), OAuthError> {
        ...
        self.persister
            .store_tokens(&token_resp.access_token, &token_resp.refresh_token)
...
mod tests {
    ...
    /// In-memory `KeychainStore` for tests, mirroring `dpop.rs`'s test double.
    struct MemKeychain;
    ...
    fn test_keypair() -> DpopKeypair {
        DpopKeypair::get_or_create::<MemKeychain>("test-oauth-client-dpop-key")
            .expect("keypair must exist")
    }
    ...
    #[tokio::test]
    async fn lazy_refresh_transport_failure_records_breadcrumb() {
        struct RecordingObserver {
            hits: Mutex<Vec<String>>,
        }
        impl TransportObserver for RecordingObserver {
            fn record_transport(&self, op: &str, _url: Option<&str>, _error: &reqwest::Error) {
                self.hits.lock().unwrap().push(op.to_string());
            }
            fn record_server(
                &self,
                _op: &str,
        ...
        assert_eq!(observer.hits.lock().unwrap().as_slice(), ["oauthRefresh"]);
    }
    ...
    async fn bearer_refresh_hits_refresh_session_not_oauth_token() {
        ...
        // Also verifies: refresh updates session in memory only (not calling the persister).
        let server = MockServer::start();
    ...
    async fn dpop_non_nonce_400_returns_response_not_notauthenticated() {
        ...
        // This is what lets the migration source path's classifier produce an XrpcError instead
        // of an opaque "Not authenticated".
        let server = MockServer::start();
        let mock = server.mock(|when, then| {
            when.method(GET)
                .path("/xrpc/com.atproto.server.getServiceAuth");
