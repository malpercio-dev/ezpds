...
- `just ticket-ref-check` - Fail if Rust source or AGENTS.md files carry a Linear ticket / AC reference (scripts/ticket-ref-check.sh; traceability belongs in ADRs and design/test plans under `docs/`)
...
- `just capability-docs-check` - Verify the operator capability documentation hasn't drifted from the capability table in `crates/pds/src/capabilities.rs` (scripts/capability-docs-check.sh; the table is the source of truth for what `describeServer` advertises under the `custos` extension and which config gates each capability — the gate fails if a capability is added, renamed, or re-gated without `sites/docs/src/content/docs/operator/capabilities.md` naming it and its controls in the same change; the generated reference table is covered by `just docs-check`)
...
- `just bundle-identity-check` - Refuse an iOS bundle-id rename that arrives without its migration (scripts/bundle-identity-check.sh; pins both apps' identifiers, asserts the wallet's stable Keychain access group is declared **first** with the legacy group still present, and asserts the iCloud container id stays frozen). The bundle id used to double as the Keychain access group *and* the iCloud container id, so a rename would have made every device key, recovery share, session, and backup unreachable with no error — see [ADR-0030](docs/architecture/decisions/0030-stable-keychain-group-and-frozen-icloud-container.md); the script's header carries the rename checklist
...
- **Token layer (code):** `apps/identity-wallet/src/lib/styles/{tokens,fonts,base}.css` is the live design system — global OKLCH color/type/space/radius/motion tokens, self-hosted fonts (bundled in `static/fonts/`, no runtime CDN), and base styles. `base.css` carries two load-bearing globals: a `button, input, select, textarea { font: inherit }` reset — without it the UA's `font: 400 13.333px Arial` shorthand wins for every control *and its descendants*, which silently rendered every hand-rolled button in the app (identity cards, nav rows, strips) in Arial while the shared `Button`/`TextField` primitives, which set the family themselves, looked correct — and a `.sr-only` clip utility for restating in words any meaning carried only by a shimmer, colour, or icon. Imported once in `src/routes/+layout.svelte`. Components reference `var(--color-*)`, `var(--font-*)`, `var(--space-*)`, etc.; **never hardcode hex or px**. Every screen has been migrated — the `src/lib/components` + `src/routes` tree is hex-free. Shared UI primitives live in `src/lib/components/ui/` (`Button`, `TextField`, `Spinner`, `SealEmblem`, `OnboardingShell`, `UrgencyBadge`, `StatusPanel`, `NavRow`, `DiffRow`, `ScreenHeader`, `SkeletonCard`, `Toggle`); reuse them rather than re-styling. `ScreenHeader` is the home-surface chrome (optional circular back + title + optional right-aligned actions; `size="home"` for the root list; `backText` swaps the icon-only circle for a labelled pill, for the rare exit that is a decision rather than a navigation — `backLabel` alone is an accessible name and stays invisible to a sighted user), `SkeletonCard` the shimmer placeholder (`seal` adds the avatar circle). `StatusPanel` is the large-format sibling of `UrgencyBadge` — the same four-state safe/warning/critical/expired vocabulary at panel scale, opening every identity screen so state leads and actions follow; `NavRow` is the one destination row every tier of the identity IA is built from (Use zone, the two doors, the everyday tier, the advanced tier), so tiers differ by depth and framing rather than by row styling. `Toggle` is the settings on/off switch — the whole row is a WAI-ARIA `switch` (≥44px target); state is carried by knob position + track fill/hollow (never colour alone), with a `prefers-reduced-motion` fallback.
...
- **No ticket or AC references in source code.** Do not add Linear ticket identifiers or numbered acceptance-criteria identifiers to `.rs` files or AGENTS.md files. ADRs, design plans, and test plans in `docs/` are the right home for ticket traceability. Source code comments should describe *why* in terms of the system, not which ticket required it.
