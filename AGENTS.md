...
- **Status:** Phase 6 ([MM-190](https://linear.app/atbb/issue/MM-190/phase-6-companion-app-scaffold-admin-device-key-app)) scaffolded the Tauri app. The token layer now exists as code — `apps/admin-companion/src/lib/styles/{tokens,fonts,base}.css` (forked OKLCH, terminal-native, every text pair verified WCAG 2.2 AAA). `DESIGN.md` remains a SEED for *components* (none built yet — Phase 8). Next design step: once Phase 8 ships real screens, re-run `/impeccable document` in scan mode against `apps/admin-companion/` to extract component tokens + the `.impeccable/design.json` sidecar. App-specific contracts: [apps/admin-companion/CLAUDE.md](apps/admin-companion/CLAUDE.md).
