...
- `just ci` - Full local gate (fmt-check, lock-check, bruno-check, docs-check, changelog-check, changelog-test, font-check, cap-check, ticket-ref-check, auth-seam-check, ssrf-client-check, gc-guard-check, ios-paths-check, swift-rs-check, ios-template-check, clippy, test, audit, deny) — the same checks CI runs
...
- `just mcp-sidecar-setup` / `just mcp-sidecar <args>` / `just mcp-sidecar-test` - Install deps for, run, and test the credential-forwarding **Streamable-HTTP MCP sidecar** (`tools/mcp-sidecar/`, the hosted Custos tier); `mcp-sidecar-test` is hermetic and self-contained (stub PDS on loopback — no cargo build, no live network), also not part of `just ci`. The sidecar single-sources the `tools/mcp` tool surface and forwards the caller's credential (ADR-0024)
...
- **Status:** Phase 6 ([MM-190](https://linear.app/atbb/issue/MM-190/phase-6-companion-app-scaffold-admin-device-key-app)) scaffolded the Tauri app **and built the Brass Console design system**: the token layer (`apps/admin-companion/src/lib/styles/{tokens,fonts,base}.css`, forked OKLCH, every text pair verified WCAG 2.2 AAA) plus the canonical UI primitives in `apps/admin-companion/src/lib/components/ui/` (`Button`, `StatusChip`, `CodeOutput`, `DeviceRow`, `TextField`, `ScreenShell`), exercised at the `/preview` route and documented in `DESIGN.md` §5. Phase 7 (MM-194) added the pairing + request-signing client, **Phase 8 (MM-195) landed the operator screens**, and a follow-on multi-server phase made the app pair with several relays at once: Pair (QR/manual + required nickname), Home (biometric-gated claim code for the *active* server, tappable identity block → inline switcher), and Settings (per-server list with per-entry rename / revoke-on-server / forget-locally, global admin key, biometric toggle), plus the error-state matrix and the biometric (`tauri-plugin-biometric`) + share-sheet (`tauri-plugin-sharesheet`) mobile plugins. Pairings persist as one versioned JSON keychain item (`admin-pairings`) with a Rust-owned active pointer, so the server the UI shows and the one an unqualified command acts on cannot diverge. A July 2026 wave of operator screens (MM-251–259, MM-277) then built out the full console: device list/remote-revoke, account listing/search + detail (usage/storage), takedown/restore + credential-revocation moderation, claim/invite-code inventory, in-flight transfer visibility/cancel, and server health/metrics. The `.impeccable/design.json` sidecar now exists; still open: the on-simulator demo (needs a Mac/Xcode). App-specific contracts (IPC surface, pairing document): [apps/admin-companion/AGENTS.md](apps/admin-companion/AGENTS.md).
