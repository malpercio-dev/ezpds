...
Checkpoint: `nix develop` drops into a shell with Rust toolchain, `cargo build` succeeds, `just check` runs, CI is green on an empty workspace, `nix build` produces a relay binary.
...
1. **MM-63** — Cargo workspace setup (root Cargo.toml, crate directories: relay, repo-engine, crypto, common)
...
3. **MM-65** — Nix flake — relay build output (`nix build .#relay`). *Blocked by: MM-63*
4. **MM-135** — NixOS module for relay deployment (systemd service, relay.toml config options, user/group). *Blocked by: MM-65, MM-69*
5. **MM-66** — Docker image derived from Nix build (`nix build .#docker-image`) — single-layer OCI image, minimal base. *Blocked by: MM-65*
6. **MM-67** — Justfile with recipes: check, build, test, fmt, clippy, run-relay, nix-build, docker-build. *Blocked by: MM-63*
...
8. **MM-69** — Configuration system (relay.toml parsing, env var overrides, crate: common). *Blocked by: MM-63*
...
9. Axum HTTP server skeleton + XRPC routing (crate: relay)
...
26. Provisioning: GET /v1/devices/:id/relay (relay endpoint discovery)
...
30. Relay signing key generation — POST /v1/relay/keys (crate: crypto)
...
45. Blob storage backend — local FS, CID generation, MIME sniffing (crate: relay)
...
Checkpoint: Relay emits firehose. BGS can subscribe. Posts appear in Bluesky AppView. The relay is a federating PDS.

50. Firehose emitter — subscribeRepos WebSocket event stream (crate: relay)
...
57. Iroh tunnel integration — relay endpoint for device connections (crate: relay)
...
├── crates/
│   ├── relay/              (Axum server, XRPC, OAuth, provisioning API, blob storage)
