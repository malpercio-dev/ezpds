# MM-90 Implementation Plan — Phase 2: relay crate POST /v1/dids

**Goal:** Replace the relay-signing DID handler with a device-signing handler that validates the client-submitted signed genesis op.

**Architecture:** Imperative Shell only. `crates/relay/src/routes/create_did.rs` is completely rewritten. Calls `crypto::verify_genesis_op` (Phase 1) for all cryptographic work. All I/O (DB, HTTP) stays in the relay crate.

**Tech Stack:** Rust/axum, sqlx (SQLite), reqwest, serde_json, wiremock (tests), crypto crate (Phase 1).

**Scope:** Phase 2 of 2 (relay crate only; depends on Phase 1)
...
<!-- START_TASK_1 -->
### Task 1: Replace handler, types, and `build_did_document` in `create_did.rs`

**Verifies:** MM-90.AC2.1, MM-90.AC2.2, MM-90.AC2.3, MM-90.AC2.4, MM-90.AC2.5, MM-90.AC2.6, MM-90.AC3.1, MM-90.AC3.2, MM-90.AC3.3, MM-90.AC3.4, MM-90.AC3.5, MM-90.AC3.6, MM-90.AC3.7, MM-90.AC4.1, MM-90.AC4.2, MM-90.AC4.3

**Files:**
- Modify: `crates/relay/src/routes/create_did.rs` (full replacement of comment block, imports, structs, handler, and `build_did_document`; keep `// ── Tests ──` section intact until Task 3)

**Implementation:**

Replace everything from line 1 through the end of `build_did_document` (keep the test block). Preserve the `// pattern: Imperative Shell` comment convention, updated for MM-90.

**1. Updated comment block and imports (replace lines 1–35):**
...
**2. New request/response structs (replace lines 37–48):**
...
**3. New handler body (replace lines 50–306):**
...
**4. Updated `build_did_document` function (replace lines 308–348):**
...
Run: `cargo clippy -p relay -- -D warnings`
Expected: Zero warnings

**Commit:** `feat(relay): replace POST /v1/dids with device-signing handler (MM-90 Phase 2, step 1)`
<!-- END_TASK_1 -->

<!-- START_TASK_2 -->
### Task 2: Update `bruno/create-did.bru` for new request shape

**Verifies:** None (Bruno file is developer tooling, not covered by ACs)

**Files:**
- Modify: `bruno/create-did.bru` (resolve merge conflict, update request body)

**Implementation:**

Replace the entire file contents with the following (resolves the existing merge conflict markers and updates to MM-90 request shape):
...
Open Bruno desktop app or inspect the file. Confirm no merge conflict markers (`<<<<<<<`, `=======`, `>>>>>>>`) remain. Confirm request body uses `rotationKeyPublic` and `signedCreationOp`.

**Commit:** `chore(bruno): update create-did.bru for MM-90 device-signing request shape`
<!-- END_TASK_2 -->
...
<!-- START_TASK_3 -->
### Task 3: Replace all inline tests with MM-90 AC coverage

**Verifies:** MM-90.AC2.1, MM-90.AC2.2, MM-90.AC2.3, MM-90.AC2.4, MM-90.AC2.5, MM-90.AC2.6, MM-90.AC3.1, MM-90.AC3.2, MM-90.AC3.3, MM-90.AC3.4, MM-90.AC3.5, MM-90.AC3.6, MM-90.AC3.7, MM-90.AC4.1, MM-90.AC4.2, MM-90.AC4.3

**Files:**
- Modify: `crates/relay/src/routes/create_did.rs` — replace entire `#[cfg(test)] mod tests` block (from `// ── Tests ──` to end of file)

**Implementation:**

Delete everything from the `// ── Tests ──` comment (currently line 350) to the end of the file, and replace with the following test module.

**Key changes from MM-89 test module:**
- Remove `TEST_MASTER_KEY` constant (no relay key to decrypt)
- Remove relay signing key insertion from `insert_test_data`
- Remove `signing_key_master_key` from `test_state_for_did` (simplify to wrap `test_state_with_plc_url`)
- Add `make_signed_op(handle, public_url)` helper using `build_did_plc_genesis_op`
- Update `create_did_request` to accept `rotation_key_public` and `signed_creation_op`
- Replace all 7 MM-89 test functions with 9 MM-90 test functions

**Test module to write:**
...
Expected: All tests pass. All 7 MM-89 test functions are removed; 9 new MM-90 test functions pass.

Run: `cargo test --workspace`
Expected: All workspace tests pass (crypto + relay).

Run: `cargo clippy --workspace -- -D warnings`
Expected: Zero warnings across all crates.

**Commit:** `test(relay): replace MM-89 tests with MM-90 AC2-AC4 coverage for POST /v1/dids`
<!-- END_TASK_3 -->
