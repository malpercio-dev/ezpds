...
|---|-------|----------|---------|
| 1.3 | **Signing key custody** — architecture says device signs; mobile says PDS always signs | **PDS always signs.** Desktop constructs Merkle tree, PDS signs commits. Desktop-local signing is a future option (mobile spec §10). | architecture (data flow, PDS shell desc, v0.1 scope) |
...
| 1.2 | **DID ceremony** — provisioning says client builds full DID doc; mobile says client sends keys, PDS assembles | **PDS constructs it.** Client sends key material, PDS orchestrates did:plc creation. | provisioning (POST /v1/dids request body) |
| 1.4 | **Tier naming** — architecture has Free/Pro/Business; provisioning has Free/Pro/Self-Hosted | **Three tiers + BYO.** Free/Pro/Business are subscription tiers. Self-Hosted (BYO PDS) is an orthogonal deployment model, not a tier. | architecture, provisioning |
| 2.5 | **Shamir shares** — migration has device/PDS/iCloud; mobile has iCloud/PDS/BIP-39 phrase | **User chooses Share 3.** Share 1 = iCloud Keychain, Share 2 = PDS escrow, Share 3 = user's choice of device-local OR BIP-39 paper/USB. | migration, mobile |
...
| 2.8 | **Desktop offline writes** — architecture implies offline compose+sync; mobile says 503 | **Resolved by signing decision.** PDS signs all commits, so writes return 503 when desktop is offline (PDS can't construct commits from stale state). Architecture needs updating. | architecture |
...
- POST /v1/pds/keys — generate PDS signing key
- DELETE /v1/pds/keys/:keyId — revoke PDS signing key
- GET /v1/pds/repo/snapshot — full repo snapshot (CAR)
...
- POST /v1/pds/commits/sign — sign unsigned commit
- GET /v1/pds/mode — current operating mode
...
- GET /v1/recovery/restore — stream repo + blobs from PDS
- PUT /v1/keys/shares/:id — update PDS-held Shamir share
- GET /v1/keys/rotation-log — audit log of Shamir rotations

**Action needed:** Consolidate into provisioning API v0.3 or create a unified PDS API Reference.

### 2.6: Firehose in mobile-only phase

Architecture tags Firehose Proxy as v1.0 and says "BGS drops on sleep" for free tier in v0.1. But in the mobile-only phase (mobile spec), the PDS IS a full PDS and must emit firehose events from day one — there's no "sleep" because the PDS is always on.

The architecture's firehose proxy concept (PDS maintains a persistent BGS WebSocket on behalf of a desktop that sleeps) is a *desktop-enrolled* feature. In mobile-only mode, the PDS just emits firehose natively like any hosted PDS.

**Action needed:** Architecture needs to distinguish between "PDS as native PDS firehose emitter" (mobile-only, always available) and "PDS as firehose proxy for sleeping desktop" (desktop-enrolled, v1.0 paid feature).

### 2.7: No blob handling spec

No document specifies how blobs (images, media) are uploaded, stored, or served through the PDS. The migration spec discusses blob loss on free tier, but the upload/storage/proxy path is unspecified.
...
The architecture is entirely desktop-centric. The mobile-only lifecycle phase (PDS as full hosted PDS) isn't represented.

**Action needed:** Add a "Phase 0: Mobile-Only" to the architecture showing the PDS as a complete PDS before any desktop is enrolled.

### 3.3: No PDS internals spec

All four docs describe the PDS from the outside. None covers database schema, process architecture, deployment model, or scaling strategy.
...
**Action needed:** Architecture should move basic Shamir to v0.1 (or acknowledge it's a PDS-side feature available from mobile launch).
...
Mobile v0.1 has no architecture milestone — it's PDS-only.
...
1. Update signing model: device constructs, PDS signs
2. Update data flow diagram: show unsigned commit → PDS → signed commit
3. Update PDS shell description: "repo construction engine" not "owns XRPC surface"
4. Add "Phase 0: Mobile-Only" lifecycle phase
5. Fix tier model: Free/Pro/Business + BYO PDS as deployment option
...
16. Update Shamir share model: Share 1=iCloud, Share 2=PDS, Share 3=user's choice
