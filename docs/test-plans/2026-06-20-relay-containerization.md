...
| B3 | AC1.3 image — no libssl3 | `docker run --rm --entrypoint /bin/sh ezpds-relay:dev -c "dpkg -l libssl3 2>/dev/null \| grep -q '^ii' && echo LIBSSL_PRESENT \|\| echo NO_LIBSSL3"` | `NO_LIBSSL3` — `ca-certificates` installs the `openssl` CLI tool (expected, not a link dep); the shared library `libssl3` must be absent. |
