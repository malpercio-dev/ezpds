...
**The blob mirror replicates; the blob scrub verifies.** A second periodic sweep re-hashes every stored blob against its `blobs` row (hash + size) and walks the blob directory for both orphan directions nothing else scans for — a row whose file is missing (the fault that blocks a migration drain mid-run) and a file no row owns. It needs no configuration to run — it is on by default at `EZPDS_BLOB_SCRUB_INTERVAL_SECS` (default 21600, i.e. every 6 hours; re-hashing every blob is I/O-heavy, so it runs far less often than the reference-reconciling blob GC). When the blob mirror is configured, a bad or missing file is auto-healed from the bucket's verified-good copy; set `EZPDS_BLOB_SCRUB_AUTO_HEAL=false` to flag such files without repairing them (auto-heal has no effect anyway when the mirror is disabled — a bad file is then only ever flagged). Health signals: `blob_scrub_flagged_total` (unhealed integrity problems — missing files, hash/size mismatches, orphan files; **the operator alarm** — a non-zero and climbing count means blobs are rotting or leaking), `blob_scrub_healed_total`, and `blob_scrub_last_run_timestamp` on `/metrics`, plus the `sweeps.blobScrub` entry in `GET /v1/admin/health`. `getBlob`'s serve-time re-hash feeds the same `blob_scrub_flagged_total` channel, so one counter covers both the background sweep and on-demand serves.
