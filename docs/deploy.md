...
- **`EZPDS_WAITLIST_ENABLED`** (optional, default `false`) - Opt-in for the public interest-signup waitlist (`POST /waitlist`, the `waitlist` capability): an unauthenticated, CORS-open endpoint a marketing page posts email + optional atproto handle to, read back by the operator via `GET /v1/admin/waitlist`. The per-IP signup cap is `EZPDS_RATE_LIMIT_WAITLIST_PER_5MIN` (default 30).
...
| `EZPDS_WAITLIST_ENABLED` | Railway dashboard | Per-environment opt-in; enable on production to open the marketing-site TestFlight waitlist |
...
> The operator-facing walkthrough of this same material is published at
> [sites/docs — Backups](../sites/docs/src/content/docs/operator/backups.md).
> Unlike the master-key runbook, no parity gate links the two copies — when you
> change the backup/restore story here, update the published page too.
...
**Litestream covers only the database.** Blob files (`{data_dir}/blobs/…` on the same volume) are replicated by the PDS's own **blob mirror**: setting `EZPDS_BLOB_MIRROR_BUCKET` — together with `EZPDS_BLOB_MIRROR_ENDPOINT` and `EZPDS_BLOB_MIRROR_ACCESS_KEY_ID` / `EZPDS_BLOB_MIRROR_SECRET_ACCESS_KEY`, the same shape as the Litestream variables — enables a periodic sweep (`EZPDS_BLOB_MIRROR_SYNC_INTERVAL_SECS`, default 300) that uploads every stored blob to an S3-compatible bucket after re-hashing it against its CID, and a restore-on-boot pass that runs **before the server takes traffic**: any blob file the volume is missing is fetched from the bucket, verified against its CID, and written back, so a Litestream database restore (or outright volume replacement) heals the blob directory automatically. A blob recoverable from neither place is logged per-CID and counted in the boot summary rather than silently 404ing later. Use a **separate bucket (or at least a distinct prefix namespace) from the Litestream replica** — the mirror owns everything under its `EZPDS_BLOB_MIRROR_KEY_PREFIX` (default `blobs/`) and deletes keys there whose blob rows have been garbage-collected. Addressing defaults to virtual-hosted style like `litestream.yml` (`EZPDS_BLOB_MIRROR_FORCE_PATH_STYLE=true` for path-style providers, e.g. MinIO), and `EZPDS_BLOB_MIRROR_REGION` defaults to `auto` (set the real region where the provider requires one). Health signals: `blob_mirror_synced_total` / `blob_mirror_last_run_timestamp` on `/metrics` and the `sweeps.blobMirror` entry in `GET /v1/admin/health` — a stale timestamp means passes are not completing.

**The blob mirror replicates; the blob scrub verifies.** A second periodic sweep re-hashes every stored blob against its `blobs` row (hash + size) and walks the blob directory for both orphan directions nothing else scans for — a row whose file is missing (the fault that blocks a migration drain mid-run) and a file no row owns. It needs no configuration to run — it is on by default at `EZPDS_BLOB_SCRUB_INTERVAL_SECS` (default 21600, i.e. every 6 hours; re-hashing every blob is I/O-heavy, so it runs far less often than the reference-reconciling blob GC). When the blob mirror is configured, a bad or missing file is auto-healed from the bucket's verified-good copy; set `EZPDS_BLOB_SCRUB_AUTO_HEAL=false` to flag such files without repairing them (auto-heal has no effect anyway when the mirror is disabled — a bad file is then only ever flagged). Health signals: `blob_scrub_flagged_total` and `blob_scrub_healed_total` on `/metrics` are **cumulative counters** — each completed pass *adds* its findings and they are never decremented, so a healed file bumps `healed` without lowering `flagged`, and a past incident leaves `flagged` permanently non-zero. **Alert on `blob_scrub_flagged_total`'s rate of increase** (new integrity problems appearing between passes), not on any non-zero value. For the *current* picture, read the `sweeps.blobScrub` entry in `GET /v1/admin/health`: its `swept` is the **most recent pass's** flagged count (missing files, hash/size mismatches, orphan files), and its `completedAt` — mirrored by `blob_scrub_last_run_timestamp` — going stale is the signal that passes are not completing. `getBlob`'s serve-time re-hash feeds the same cumulative `blob_scrub_flagged_total` channel, so that one counter covers both the background sweep and on-demand serves.
...
- **Full operator step ordering for both the loss and the compromise scenario** — which rows to drop, and the order to rotate secrets in — lives in the [Master-Key (KEK) Disaster Runbook](operations/master-key-disaster-runbook.md). This section is the prevention-and-pointers summary; the runbook is the incident checklist. An operator-facing rewrite of the same runbook, for operators who haven't cloned the repo, is published at [docs.obsign.org/operator/master-key-runbook](https://docs.obsign.org/operator/master-key-runbook/).
