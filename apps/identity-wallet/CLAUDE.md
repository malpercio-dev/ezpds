...
- `PdsClientError` variant names serialize as SCREAMING_SNAKE_CASE -- the full Rust enum is HANDLE_NOT_FOUND, DID_NOT_FOUND, PDS_UNREACHABLE, NETWORK_ERROR, INVALID_RESPONSE, OAUTH_FAILED, DID_ALREADY_EXISTS, INVALID_CREDENTIALS, AUTH_FACTOR_TOKEN_REQUIRED, INSECURE_PDS_URL. The first six can reach the frontend directly (discovery/resolve paths); DID_ALREADY_EXISTS is migration-createAccount-internal; INVALID_CREDENTIALS / AUTH_FACTOR_TOKEN_REQUIRED / INSECURE_PDS_URL are claim-flow-internal — `authenticate_source_pds` maps them into `ClaimError` (SOURCE_AUTH_FAILED / TWO_FACTOR_REQUIRED / INSECURE_SOURCE_URL), so they are never surfaced as `PdsClientError` to the frontend
