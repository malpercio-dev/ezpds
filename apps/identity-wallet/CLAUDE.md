...
- `ClaimError` variant names serialize as SCREAMING_SNAKE_CASE to the frontend -- the TypeScript `ClaimError` union must match exactly (INVALID_TOKEN, VERIFICATION_FAILED, PLC_DIRECTORY_ERROR, UNAUTHORIZED, OAUTH_REJECTED, NETWORK_ERROR). OAUTH_REJECTED carries the authorization server's own `error: error_description` text from a refused PAR, so a policy rejection (e.g. bsky.social's invalid_redirect_uri) is diagnosable in the UI instead of reading as a network failure
