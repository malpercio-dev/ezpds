...
- Rust backend / frontend -> `tauri-plugin-auth-session` (**vendored** in `src-tauri/vendor/tauri-plugin-auth-session/`; ASWebAuthenticationSession for the in-app OAuth session — replaced `tauri-plugin-deep-link` + `tauri-plugin-opener`, both removed)
...
- `src-tauri/vendor/tauri-plugin-auth-session/` -- vendored Tauri plugin (ASWebAuthenticationSession); path dep of src-tauri, excluded from the workspace; see VENDORED.md for provenance + audit
