...
- **AppState with Mutex<Option>**: `pending_login` (create flow) and `pending_source_login` (outbound migration) hold the PKCE verifier + CSRF state between the `prepare_*` and `complete_*` commands while the auth session runs; `oauth_session` holds the active tokens. All use `Mutex<Option<T>>` so the state is cleanly empty before/after flows. (The claim flow's source login is password-based — `claim::authenticate_source_pds` — so it needs no pending-login slot.)
...
- **Claim source login is password-based, not OAuth (ADR-0021/MM-289)**: the claim flow's next steps are PLC/identity operations that a spec-strict PDS (bsky.social) gates behind a full session; the atproto OAuth ceiling for a third-party client is `transition:generic`, which grants no identity ops. So `claim::authenticate_source_pds` does a one-shot password `createSession` → full-session Bearer client (`OAuthClient::new_bearer`). The **create flow** (`oauth::prepare_oauth_flow`/`complete_oauth_flow`) and the **outbound migration** source login (`migration_orchestrator::prepare_source_auth`/`complete_source_auth`) still use the in-app auth session (`plugin:auth-session|start`) — OAuth is fine for what those do. Only one auth session runs at a time.
...
- `ClaimError` variant names serialize as SCREAMING_SNAKE_CASE to the frontend -- the TypeScript `ClaimError` union must match exactly (INVALID_TOKEN, VERIFICATION_FAILED, PLC_DIRECTORY_ERROR, UNAUTHORIZED, SOURCE_AUTH_FAILED, INSUFFICIENT_SCOPE, NETWORK_ERROR). SOURCE_AUTH_FAILED is a rejected password `createSession` on the source PDS (wrong password / app-password used); INSUFFICIENT_SCOPE is a PLC-op endpoint refusing the token for scope reasons, surfaced distinctly rather than masquerading as "failed to send verification email" (the MM-289 error-surfacing fix)
