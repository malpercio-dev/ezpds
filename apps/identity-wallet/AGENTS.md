...
| `oauth.rs` | `AppState` (all flow-state slots) + `DPoPKeypair`/`OAuthSession`; the create-flow OAuth login (`prepare/complete_oauth_flow`) was removed once the create flow started ending at `home` with no OAuth round trip — slot inventory and the retirement rationale in the module doc |
...
| `base64url.rs` | the one base64 alphabet this crate uses — unpadded base64url (RFC 4648 §5) via `b64url_encode`/`b64url_decode`, for the DPoP proofs, JWT segments, and JWK coordinates in `oauth.rs`/`oauth_client.rs`; centralizes the `URL_SAFE_NO_PAD` engine choice so a call site never re-hand-rolls it — see module doc |
