...
- `src/lib/components/home/` — eight home screen components (IdentityListHome, DIDDocumentScreen, AlertDetailScreen, RecoveryOverrideScreen, MyAgentsScreen, AgentDetailScreen, AgentClaimApprovalScreen, SettingsScreen) plus DIDAvatar utility component (deterministic DID-derived hue circle). MyAgentsScreen is the agent list; AgentDetailScreen is the per-agent sub-view (audit-trail paging + biometric-gated revoke) it renders on selection, its lifetime scoped to that selection
...
- The `src/lib/ipc/` modules are the only files that call `invoke()` directly (each domain module — account, oauth, appearance, claim, identity, monitor, recovery, migration, agents — re-exported from `src/lib/ipc/index.ts`); page components import from `$lib/ipc`. `authenticateBiometric` is a policy gate, not an `invoke()` wrapper, and lives in `$lib/biometric.ts` (matching the sibling admin-companion app's layout)
...
- `src/lib/ipc/` -- Per-domain typed TypeScript wrappers for all Tauri IPC commands, split into `account.ts`, `oauth.ts`, `appearance.ts`, `claim.ts`, `identity.ts`, `monitor.ts`, `recovery.ts`, `migration.ts`, `agents.ts` (plus the generic `isCodedError` IPC-error guard in `errors.ts`), all re-exported from `index.ts`; page components import from `$lib/ipc`
...
- `src/lib/components/onboarding/` -- Seventeen onboarding screen components (ModeSelectScreen, PdsConfigScreen, ClaimCodeScreen, EmailScreen, HandleScreen, PasswordScreen, LoadingScreen, DIDCeremonyScreen, DIDSuccessScreen, ShamirBackupScreen, HandleRegistrationScreen, AuthenticatingScreen, IdentityInputScreen, PdsAuthScreen, EmailVerificationScreen, ReviewOperationScreen, ClaimSuccessScreen)
- `src/lib/components/home/` -- Eight home screen components (IdentityListHome, DIDDocumentScreen, AlertDetailScreen, RecoveryOverrideScreen, MyAgentsScreen, AgentDetailScreen, AgentClaimApprovalScreen, SettingsScreen) plus DIDAvatar utility component
- `src/lib/deadline.ts` -- PLC recovery deadline utilities (getDeadline, getUrgency, formatCountdown); tested by `deadline.test.ts`
- `src/routes/+page.svelte` -- Two-flow state machine starting at mode_select; Create flow: mode_select -> pds_config -> ... -> home; Import flow: mode_select -> identity_input -> pds_auth -> email_verification -> review_operation -> claim_success -> home; Home: home (IdentityListHome) -> identity_detail -> alert_detail -> recovery_override, plus home -> settings (SettingsScreen, appearance control); visibilitychange handler calls checkIdentityStatus() on foreground
