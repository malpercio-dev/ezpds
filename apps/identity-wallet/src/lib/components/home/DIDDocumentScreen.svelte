<script lang="ts">
  ...
  // The DID document, as the inspection and verification surface it is named for.
  //
  // This screen used to be the identity hub: every verb the wallet could perform on an
  // identity hung off the bottom of it, sorted by nothing. The verbs now live on the
  // instrument-panel identity screen, tiered by how often and in what state a person
  // reaches for each; what is left here is the document itself — what the public record
  // says about this identity, in a form you can read and copy.
  let {
    didDoc,
    onback,
  }: {
