 ...
 * from it. The harness is reached only through a dynamic import gated on
 * `import.meta.env.DEV` in `src/hooks.client.ts`; Vite replaces that with `false` in a
 * production build, so Rollup drops the import and the whole harness chunk is tree-shaken
 * out. This fails loudly if that guarantee ever regresses.
 *
 * Usage: `pnpm check:harness-absence`.
 */
...
const MARKER = '__EZPDS_ADMIN_HARNESS_PRESENT__';
