...
mod algos {
    ...
    pub async fn split_subtree(
        ...
        //
        // PATCH(ezpds): upstream only rebuilt a side that already existed at the level
        // below, so when the deepest split left one side empty (`None`) every parent
        // level's entries on that side — including whole subtrees — were silently
        // dropped from the tree (atrium-rs/atrium#343). Each level must instead carry
        // its orphaned sibling entries into a new node on that side, whether or not the
        // level below contributed one.
        for (mut parent, i) in node_path.into_iter().rev() {
            ...
            left = match left {
                ...
                None if !e_left.is_empty() => Some(Node { entries: e_left.to_vec() }),
                None => None,
            };
            ...
            right = match right {
                ...
                None if !e_right.is_empty() => Some(Node { entries: e_right.to_vec() }),
                None => None,
            };
        }
