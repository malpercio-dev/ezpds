...
of these, and a well-behaved client falls back to standard AT Protocol behavior.
...
An **empty** password is refused whether or not this is enabled: an empty
string is what an uninitialized form field sends, and accepting one would let a
client bug create an account with no credential that nobody asked for. Omitting
the field is the only way to ask.
...
accounts that already have no password, which keep working as before.
...
factors rather than removing one.
...
and honoring only the weaker one would make the endpoint answer differently
...
returns the same success and never discloses whether an address was already on
the list), and the handle is syntax-checked but never resolved, so the endpoint
has no outbound-request surface. The operator reads the list back with
...
If a "Controlled by" line above disagrees with your server's behavior, that is a
