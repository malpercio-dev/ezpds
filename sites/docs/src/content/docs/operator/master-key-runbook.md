...
description: The operator incident procedure for a lost or compromised KEK.
---
...
is recoverable, restore the environment variable and boot normally, then
**stop here**: this is a misplaced env var, not a disaster. Only proceed if
the KEK is genuinely unrecoverable.
...
:::note[Takeaway]
A lost KEK is nearly as bad as a compromised one, because of the repo keys:
loss and compromise share the same hard recovery path (per-account
...
1. **`EZPDS_ADMIN_TOKEN`**, the break-glass bearer credential. Rotate it
