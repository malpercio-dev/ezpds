...
> Generated from source for ezpds **v0.6.0**. Do not edit this page by hand.
...
| `rate_limit.oauth_consent_create_per_5min` | `EZPDS_RATE_LIMIT_OAUTH_CONSENT_CREATE_PER_5MIN` | `u64` | Wallet-confirmed OAuth consent request creations (`GET /oauth/authorize` for a passwordless account) per 5 minutes. Charged in-handler against **both** the requesting IP and the `client_id` (two independent counters, one limiter), so neither a single IP nor a single client can flood the pending-request table. Default 30. `0` disables. |
