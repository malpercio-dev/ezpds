...
if [ -n "${LITESTREAM_S3_BUCKET:-}" ]; then
  ...
  # pds under Litestream so the WAL is streamed continuously to object storage.
  gosu relay litestream restore -if-db-not-exists -if-replica-exists -config /etc/litestream.yml /data/relay.db
  exec gosu relay litestream replicate -config /etc/litestream.yml -exec "/usr/local/bin/pds"
else
  # No replica configured (staging/local): original behavior, unchanged.
  exec gosu relay /usr/local/bin/pds
fi
