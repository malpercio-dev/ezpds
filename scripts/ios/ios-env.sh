#!/usr/bin/env bash
# ios-env.sh — derive the Apple toolchain for cross-compiling the iOS apps'
# (identity-wallet, admin-companion) Rust code, with ZERO hardcoded paths.
#
...
# as before. The iOS build entry points set EZPDS_IOS_BUILD=1: the `just ios-*`/
# `just admin-*` recipes and the injected Xcode "Build Rust Code" Run Script block.
